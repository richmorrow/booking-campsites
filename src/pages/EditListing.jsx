...
export default function CreateListing() {
  ...
  useEffect(() => {
    ...
    async function fetchListing() {
      ...
      if (docSnap.exists()) {
        ...
        navigate("/");
        toast.error("Объявления не существует");
      }
    }
    fetchListing();
  ...
  async function onSubmit(e) {
    ...
    if (discountedPrice >= regularPrice) {
      setLoading(false);
      toast.error("Цена со скидкой должна быть ниже обычной цены");
      return;
    }
    if (images.length > 6) {
      setLoading(false);
      toast.error("Возможна загрузка не более 6-ти фотографий");
      return;
    }
    let geolocation = {};
    let location;
    if (geolocationEnabled) {
      ...
      if (location === undefined) {
        setLoading(false);
        toast.error("пожалуйста, укажите правильный адрес");
        return;
      }
    } else {
  ...
  return (
    <body className="w-full bg-cover bg-center bg-[url(https://images7.alphacoders.com/717/717815.jpg)]">
      <main className="max-w-md px-4 mx-auto bg-gray-100 bg-opacity-90">
        <h1 className="text-3xl text-center pt-4 font-bold">Редактирование</h1>
        <form onSubmit={onSubmit}>
          ...
          {!geolocationEnabled && (
            <div className="flex space-x-6 justify-start mb-6">
              <div className="">
                <p className="text-lg font-semibold">Широта</p>
                <input
                  type="number"
                  id="latitude"
                  value={latitude}
              ...
              <div className="">
                <p className="text-lg font-semibold">Долгота</p>
                <input
                  type="number"
                  id="longitude"
                  value={longitude}
